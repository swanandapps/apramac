...
<el-button id='deletebtn' @click="changename(product,index)">Change Name</el-button>
<el-button id='deletebtn' @click="changedescription(product,index)">Change Description</el-button>
...
export default {
  ...
  methods: {
     changeprice(product,index) {
        MessageBox.prompt('Please Input New Price', 'Tip', {
...
.catch(function(error) {
    //////console.error("Error writing document: ", error);
          ...
          Message({
           ...
           changename(product,index) {
        MessageBox.prompt('Please Input New Name', 'Tip', {
          confirmButtonText: 'OK',
          cancelButtonText: 'Cancel',
         
        }).then(value => {
            this.changed_name=value,
            //////console.log(this.changed_name.value)
 this.product_id = product.id;
      let tempthis = this;
      //////console.log(product.id);
  ...
  db.collection("products").doc(product.id).update({
    name:tempthis.changed_name.value
})
.then(function() {
    //////console.log("Document successfully written!");
    tempthis.$swal("Updated!", "Product Name is Updated", "success");
})
.catch(function(error) {
    //////console.error("Error writing document: ", error);
});
   ...
   changedescription(product,index) {
        MessageBox.prompt('Please Input New Description', 'Tip', {
          confirmButtonText: 'OK',
          cancelButtonText: 'Cancel',
         
        }).then(value => {
            this.changed_description=value,
            //////console.log(this.changed_description.value)
 this.product_id = product.id;
      let tempthis = this;
      //////console.log(product.id);
  ...
  db.collection("products").doc(product.id).update({
    description:tempthis.changed_description.value
})
.then(function() {
    //////console.log("Document successfully written!");
    tempthis.$swal("Updated!", "Product Description is Updated", "success");
})
.catch(function(error) {
    //////console.error("Error writing document: ", error);
});
...
/*remaining firestore
  beforeCreate() {
    axios
      .get("https://aprakrta-48342.dbio.com/userdata.json")
      .then(res => {
        const data = res.data;
